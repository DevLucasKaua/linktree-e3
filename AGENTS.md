...
- **Painel admin**: linguagem KAPTA — tema claro/escuro via `[data-theme]` (tokens em `src/app/globals.css`), acento `#FF4D00`, componentes compartilhados em `src/components/ui/`. Usar sempre `Button`/`Field`/`Section`/`Badge`/`Segmented`/`Menu`/`Modal` e `useToast`/`useConfirm` (nunca `alert()`/`confirm()`).
